...
function LoginPage({ session, saveSession, setOutput }: AuthProps) {
  const navigate = useNavigate()

  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    try {
      const values = data(event.currentTarget)
...
function SignupPage({ session, saveSession, setOutput }: AuthProps) {
  const navigate = useNavigate()
  ...
  return (
    <Card title="Create Account" subtitle="Register a new user then continue to main dashboard.">
      <form className="form" onSubmit={onSubmit}>
        <InputField label="Email" name="email" type="email" />
        <InputField label="Password" name="password" type="password" />
...
function MainPage({ session, saveSession, setOutput }: MainProps) {
  const submit =
  ...
  return (
    <div className="app-shell">
      ...
      <main className="content">
        <Routes>
          <Route path="/login" element={<LoginPage session={session} saveSession={saveSession} setOutput={setOutput} />} />
          <Route path="/signup" element={<SignupPage session={session} saveSession={saveSession} setOutput={setOutput} />} />
          <Route path="/main" element={<MainPage session={session} saveSession={saveSession} setOutput={setOutput} />} />
